...
public class Time1
{
    ...
    public Time1 addMinutes (int num){
        int m=_minute;
        int hoursToMinutes=_hour*MININHOUR; //transfer the hours in the time object to minutes
        int comb=num+hoursToMinutes+m; //combines the given num with the time object
        int totalHours=comb/MININHOUR; //gets the hour parameter for the new time object
        if (totalHours<MIN) //checks if the total hours is less than 0 then if yes adds 24 hours to get valid time
            totalHours+=HOURSINDAY;
        else
            totalHours=totalHours;
        int totalMinutes=comb%MININHOUR;
        if (totalMinutes<MIN) //checks if the minutes is less than 0 if yes adds 60 minutes to get valid time
            totalMinutes+=MININHOUR;
        else
            totalMinutes=totalMinutes;
        int dayHoursFormat=totalHours%HOURSINDAY; //transfter the hours to 24 hours time frame
        int h2=dayHoursFormat; //hours parameter for new time
        int m2=totalMinutes; //minutes parameter for new time
        Time1 t2=new Time1(h2, m2); //creates new time object to return
        return t2;
    }
}